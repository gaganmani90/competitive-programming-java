...
/**
 * 1561. Maximum Number of Coins You Can Get My SubmissionsBack to Contest
 * User Accepted: 5149
 * User Tried: 5398
 * Total Accepted: 5252
...
public class MaxNumberOfCoins {
    public static int maxCoins(int[] piles) {
        Arrays.sort(piles);
        int count = 0;
        int bobIndex = 0, aliceIndex = piles.length-1, index = aliceIndex-1;

        while(bobIndex <= index-1) {
            count += piles[index];
            index-=2;
            aliceIndex-=2;
            bobIndex++;
    ...
    public static void main(String[] args) {
        ...
        Assert.assertEquals(4, maxCoins(new int[]{2,4,5}));
